...
{capture name=ebAccountBreadcrumb}
    <div class="eb-breadcrumb">
        ...
        <span class="eb-breadcrumb-current">Add Contact</span>
    </div>
{/capture}
...
{capture name=ebAccountContent}
    {include file="$template/includes/ui/page-header.tpl"
        ...
        ebPageTitle={lang key='clientareanavaddcontact'}
        ebPageDescription="Create a new contact record and configure who receives different account notifications."
    }
    ...
    {if $successMessage}
        {include file="$template/includes/ui/eb-alert.tpl" ebAlertType="success" ebAlertMessage=$successMessage}
    {/if}
                ...
                <div class="relative" x-data="ebSelectMenu({ selectId: 'inputContactId', placeholder: '{lang key='clientareachoosecontact'|escape:'javascript'}' })" x-init="init()" @click.outside="close()" @keydown.escape.prevent="close()">
                    ...
                    <div x-show="open"
                        ...
                        <div class="max-h-64 overflow-y-auto">
                            <template x-for="option in options" :key="'contact-new-selector-' + option.value">
    ...
    <div class="mt-4 eb-subpanel overflow-visible">
        <form role="form" method="post" action="{routePath('account-contacts-new')}" class="space-y-6">
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div class="relative" x-data="ebSelectMenu({ selectId: 'country', placeholder: '{lang key='clientareacountry'|escape:'javascript'}' })" x-init="init()" @click.outside="close()" @keydown.escape.prevent="close()">
                        ...
                        <div x-show="open"
                            ...
                            <div class="mt-2 max-h-64 overflow-y-auto">
                                <template x-for="option in filteredOptions" :key="'contact-new-country-' + option.value">
                                    <button type="button" class="eb-menu-item w-full" :class="selectedValue === option.value ? 'is-active' : ''" @click="select(option.value)">
                                        <span class="min-w-0 flex-1 truncate text-left" x-text="option.label"></span>
                                    </button>
                                </template>
                ...
                <div>
                    ...
                    <div id="contactNewStateWrapper"
                        class="relative"
                        x-data="ebStateSelectMenu({ rootId: 'contactNewStateWrapper', countryId: 'country', placeholder: '{lang key='clientareastate'|escape:'javascript'}' })"
                        x-init="init()"
                        @click.outside="close()"
                        ...
                        <template x-if="hasSelect">
                            <div>
                                ...
                                <div x-show="open"
                                    ...
                                    style="display: none;">
                                    <div class="max-h-64 overflow-y-auto">
                                        <template x-for="option in options" :key="'contact-new-state-' + option.value">
                                            <button type="button" class="eb-menu-item w-full" :class="selectedValue === option.value ? 'is-active' : ''" @click="select(option.value)">
                                                <span class="min-w-0 flex-1 truncate text-left" x-text="option.label"></span>
                                            </button>
                                        </template>
