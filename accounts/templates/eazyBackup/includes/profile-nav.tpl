...
<nav class="flex flex-wrap items-center gap-1" aria-label="Account Navigation">
    ...
    <a href="{$WEB_ROOT}/index.php?m=eazybackup&a=inbox" class="eb-tab{if $smarty.get.m eq 'eazybackup' && $smarty.get.a eq 'inbox'} is-active{/if}"{if $smarty.get.m eq 'eazybackup' && $smarty.get.a eq 'inbox'} aria-current="page"{/if}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-5 w-5">
