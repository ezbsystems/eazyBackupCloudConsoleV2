...
function comet_Server(array $params)
{
    try {
        ...
        return new \Comet\Server($url, $params["serverusername"], $params["serverpassword"]);
    } catch (\Exception $e) {
        return $e->getMessage();
    }
}
...
function comet_UpdateUser(array $params)
{
    ...
    $deviceQuota = 0;
    $storageQuota = $deviceQuota + ($params["configoptions"]["Additional Storage"] ?? 0);

    // Resolve the per-product Lite (reduced storage) cap from the eazybackup addon
    // settings. A non-zero cap means this product is a Lite-tier SKU: enforce a
    // hard storage quota at the user profile level, cap the device count, and
    // (in CreateAccount) also set a vault-level limit on the initial vault.
    $liteCapGb = comet_LiteCapForPid((int)($params["pid"] ?? 0));
    ...
    if ($liteCapGb > 0) {
        // Lite tier: enforce the cap at the user profile level. The Comet user
        // policy itself blocks vault creation / edit / delete from the desktop
        // client; here we enforce the cap and clamp devices to 1.
        $profile->MaximumDevices               = 1;
        $profile->AllProtectedItemsQuotaBytes  = (int)$liteCapGb * (1024 ** 3);
        $profile->AllProtectedItemsQuotaEnabled = true;

        // Belt-and-braces: also set per-vault StorageLimit on any existing
        // vaults so the cap is enforced even before the desktop client refetches
        // its policy.
        if (!empty($profile->Destinations)) {
            $bytes = (int)$liteCapGb * (1024 ** 3);
            foreach ($profile->Destinations as $guid => $destCfg) {
                $destCfg->StorageLimitEnabled = true;
                $destCfg->StorageLimitBytes   = $bytes;
                $profile->Destinations[$guid] = $destCfg;
            }
        }
    } else {
        // Standard tier (or upgrade FROM a Lite tier): legacy behaviour with the
        // computed informational storage value but quota enforcement disabled.
        // If we are transitioning out of a Lite plan (vault still has a limit
        // attached), clear those limits so the customer is no longer capped.
        ...
        if (!empty($profile->Destinations)) {
            foreach ($profile->Destinations as $guid => $destCfg) {
                if (!empty($destCfg->StorageLimitEnabled)) {
                    $destCfg->StorageLimitEnabled = false;
                    $destCfg->StorageLimitBytes   = 0;
                    $profile->Destinations[$guid] = $destCfg;
                }
            }
        }
...
/**
 * Read the Lite (reduced storage) cap (in GB) for a given WHMCS product PID.
 *
 * The mapping lives in the eazybackup addon-module setting `lite_pid_caps`,
 * stored as a CSV of `PID:GB` pairs (e.g. `102:250,103:250`). Returns 0 when
 * the PID is not registered as a Lite tier.
 */
function comet_LiteCapForPid(int $pid): int
{
    if ($pid <= 0) { return 0; }
    try {
    ...
    foreach (explode(',', $csv) as $pair) {
        ...
        [$p, $gb] = array_map('trim', explode(':', $pair, 2));
        if ((int)$p === $pid) { return max(0, (int)$gb); }
    }
    return 0;
}
