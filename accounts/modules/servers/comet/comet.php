...
function comet_CreateAccount(array $params)
{
    ...
    try {
        ...
        // Resolve Lite (reduced storage) cap for this product. When non-zero we
        // also stamp the initial vault with a hard StorageLimit so the cap is
        // honored from the very first backup job, before the desktop client has
        // even fetched its user policy.
        $liteCapGbCreate = comet_LiteCapForPid((int)($params["pid"] ?? 0));
        $liteCapBytes    = $liteCapGbCreate > 0 ? ($liteCapGbCreate * (1024 ** 3)) : 0;
        ...
        if (!empty($profile->Destinations)) {
            foreach ($profile->Destinations as $guid => $destCfg) {
                ...
                // Lite tier: also enforce a per-vault StorageLimit on the
                // newly-provisioned vault.
                if ($liteCapBytes > 0) {
                    $destCfg->StorageLimitEnabled = true;
                    $destCfg->StorageLimitBytes   = $liteCapBytes;
                    $profile->Destinations[$guid] = $destCfg;
                }
            }
        }
