...
function getUserProtectedItemsDetails($username, $serviceid) {
    ...
    // Map Comet engine identifiers to friendly type labels for display.
    $engineLabel = function($engine) {
        $map = [
            ...
            'engine1/vmware'        => 'VMware vSphere',
            'engine1/winmsofficemail' => 'Microsoft 365',
            'engine1/exchangeedb'   => 'Exchange EDB',
            'engine1/mongodb'       => 'MongoDB',
            'engine1/systemstate'   => 'System State',
            'engine1/windowssystem' => 'Windows System',
            'engine1/vsswriter'     => 'VSS Writer',
        ];
        $eng = (string)$engine;
        if (isset($map[$eng])) return $map[$eng];
        if ($eng === '') return 'Unknown';
        return $eng;
    };

    // Map device IDs to friendly names so the table can attribute each PI to a device.
    $deviceNames = [];
    if (isset($user->Devices) && is_array($user->Devices)) {
        foreach ($user->Devices as $did => $dev) {
            if (is_object($dev)) {
    ...
    // Index BackupRules by source so we can attach rule references per item.
    $rulesBySource = [];
    if (isset($user->BackupRules) && is_array($user->BackupRules)) {
        foreach ($user->BackupRules as $rid => $rule) {
            if (!is_object($rule)) continue;
            $src = (string)($rule->Source ?? '');
            if ($src === '') continue;
            if (!isset($rulesBySource[$src])) { $rulesBySource[$src] = []; }
            $rulesBySource[$src][] = [
                'id'          => (string)$rid,
    ...
    if (isset($user->Sources)) {
        foreach ($user->Sources as $sourceId => $source) {
            $totalBytes = 0;
            if (isset($source->Statistics->LastBackupJob->TotalSize)) {
                $totalBytes = (int)$source->Statistics->LastBackupJob->TotalSize;
            }
            $owner = isset($source->OwnerDevice) ? (string)$source->OwnerDevice : '';
            $protectedItems[] = [
                'id'                  => (string)$sourceId,
                'engine'              => isset($source->Engine) ? (string)$source->Engine : '',
                'type'                => $engineLabel($source->Engine ?? ''),
                'total_bytes'         => $totalBytes,
                'name'                => $source->Description ?? 'N/A',
                'size'                => $totalBytes > 0 ? comet_HumanFileSize($totalBytes) : '0',
                'ownerDeviceId'       => $owner,
                'ownerDeviceName'     => isset($deviceNames[$owner]) ? $deviceNames[$owner] : '',
                'rules'               => $rulesBySource[(string)$sourceId] ?? [],
