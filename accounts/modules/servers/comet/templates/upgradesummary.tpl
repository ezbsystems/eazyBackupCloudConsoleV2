    ...
    <tbody>
        {foreach key=num item=upgrade from=$upgrades}
            ...
            {elseif $type eq "configoptions"}
                <tr>
                    {assign var=newval value=" "|explode:$upgrade.newvalue}
                    {if strpos(strtolower($upgrade.configname), "devices") !== false}
                        <td>Total Devices: {$upgrade.originalvalue + 1} {if $upgrade.originalvalue + 1 > 1}devices{else}device{/if} to {$newval[0] + 1} {if $newval[0] + 1 > 1}devices{else}device{/if}</td>
                    {elseif strpos(strtolower($upgrade.configname), "storage") !== false}
                        <td>Addon Storage: {$upgrade.originalvalue} TB to {$newval[0]} TB</td>
                    {else}
