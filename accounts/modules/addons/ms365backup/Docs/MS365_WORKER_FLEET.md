...
**Kopia (0.4.0+):** Worker pins **Kopia v0.23.1** (Go 1.25.8+ on build host). Content/metadata caches support optional hard limits via `content_cache_limit_mib` / `metadata_cache_limit_mib` (template defaults 768/256). Index cache has no byte cap in Kopia — rely on `min_index_sweep_age_seconds` plus 0.3.81 disk pressure. Repository format v2 repos open without upgrade. Integration test: `go test -tags=integration ./internal/kopia/...` with `KOPIA_TEST_*` env vars on a dedicated test prefix.
