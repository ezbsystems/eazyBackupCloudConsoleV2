...
- **Fix (PHP 1.52.53):** Mark `pagination safety cap` non-retryable; refuse/fail exhausted resumes; hand off idle-owned claims (0 running, queued children, claim age ≥180s).
- **Fix (worker 0.4.36):** Initialize `runLogMu`/`runLogState` in `NewClient`; never lazy-replace under race.
- **Verify:** `ms365_non_retryable_error_test.php`, `ms365_batch_claim_test.php`, `go test ./internal/api ./internal/jobs` PASS; user confirmed fixed; session `9e9596` instrumentation removed.
...
- **Deploy:** Committed + pushed to `origin/main` (`2387cd77`) on dev. **Deployed fleet-wide same day** — see follow-up entry below (build job 141 → release 74, deploy job 40, 8/8 nodes on 0.4.30, fleet config v6).
...
### 2026-08-01 — Disk pressure latch fix: worker 0.4.30 built, deployed fleet-wide, fleet config v6 (follow-up)

- **Build:** Build job **141** (version_label `0.4.30`, git_ref `main`, flags `run_tests=true, auto_deploy=true, deploy_strategy=rolling`) — `go test ./...` PASS, published as dev release **150** (`sha256=06701add4e57a732a79ba43571d09e4a20f53359e1c90174bcca9e67723df442`, 33,297,941 bytes). Dev's local auto-deploy (4 dev/test nodes) succeeded immediately as part of the build job.
- **Prod sync:** `ReleaseSyncService::autoPublishAfterBuild()` pushed the release to production automatically on publish (`ms365_auto_sync_release_to_prod=on`) — prod release **74** (same sha256), browse-serve sidecar binary synced.
- **Fleet deploy:** Rolling deploy job **40** on the production fleet (release 74) — **8/8 nodes updated to 0.4.30** (`succeeded`). 5 nodes were already idle and updated within the build/sync window; the remaining 3 (9002, 9011, 9012 — busy running batch `720968eb-…`) updated via graceful drain/restart once idle, including a live **ownership hand-off of batch `720968eb-…` from 9011 → 9012** mid-rollout with no discontinuity (progressed **1840→1849 success**, `queued` 240→231, `running` 6 throughout).
- **Fleet config v6:** Saved a new config version from the current template + explicit `worker.update_reserve_mib: 256` / `worker.disk_hysteresis_mib: 512` / `kopia.content_cache_limit_mib: 768` / `kopia.metadata_cache_limit_mib: 256` (previously implicit via `applyDefaults`, v5 omitted them) — version **6**, `sha256=8abc4fad27d626b0fc3bcf0b9981aa76ed28bc637d9405ffe6056df3145e7486`. Rollout targeted all 8 nodes; 4 idle nodes applied immediately (`config_status=current`), remaining 4 busy nodes pending `applying` until their next idle cycle (same safe/rolling behavior as binary deploy).
- **Verify (prod):** `bin/ms365_fleet_smoke.php` — all checks `[OK]`, `active_worker_nodes=8`, `artifact_file`/`browse_binary` both `v0.4.30`. Batch `720968eb-…` healthy throughout (`79.68%`, 6 running, no `disk_critical`). Confirmed the actual fix live via worker journal on **9010**: `kopia cache limits applied config=….config: content soft=512MiB hard=2048MiB metadata soft=128MiB hard=512MiB` on repo open — hard caps are now genuinely persisted (previously silent no-op).
- **Deploy status:** Fleet fully on worker 0.4.30 (8/8); fleet config v6 rollout converging (4/8 applied, 4/8 pending next idle window) — no further action needed, remaining nodes will pick up v6 automatically.
...
- **Fix:** `CometPersonalSiteResolver` resolves Graph drive owners; on Graph **423 blocked site**, fall back to `sites/getAllSites` metadata and match `/personal/…` path or display name to inventory user/mailbox. Mapper selects owner with mailbox scopes + OneDrive. Report: `personal_sites_mapped_to_users`.
- **Verify:** Unit tests PASS; prod dry-run maps James (`fa2a9e71-…`) with `personal_sites_mapped_to_users=1`.
...
- **Production verification:** Batch moved to worker 9010 on 0.4.5 at attempt **79**, remained stable for more than ten minutes, then naturally handed off to worker 9013 on 0.4.5 at attempt **80**. New owner started at 11:47:33 UTC; old owner detected the ghost claim at 11:47:47 and started no later children. Fleet journals contain no `Batch lease is not active` entries for the transition; historical completion-outbox 409 replays ended at 11:44:27. Success count increased **1321 → 1324** with zero failed/error children and fresh heartbeat/progress.
...
- **Fix (worker 0.4.2):** Mail-only `paginateMailFolders` sets `SoftStopRepeatedNextLink` (separate from `DuplicatePageDetectOnly`) for repeated nextLink soft-stop (`PaginationOutcome.StoppedOnRepeatedNextLink`). SharePoint/calendar DetectOnly callers keep strict repeated-nextLink errors. Ladder retries at page sizes **100 → 50 → 25**: returns on natural completion; on wedge, logs warning and retries smaller; on final wedge, keeps unique folders returned and continues sync. Mail-folder monitor uses nil per-page logger; high-level `opts.Log` wedge warnings unchanged. Message-delta behavior unchanged.
...
- **Deploy:** Pushed `main` through `3e7aac58`; production WHMCS deployed via `deploy-production.sh`. Dev build job **113** published release **124** (`sha256 a183f2c250de6361…`), synced to production release **48**, and rolling deploy job **21** completed **8/8** nodes on 0.4.2. Production health check and fleet smoke passed; browse binary is synced at 0.4.2.
- **Cleanup (worker 0.4.3):** Operator confirmed the production issue fixed. Removed all session `6f5f7c` mail diagnostics and deleted the session NDJSON log; recovery behavior and regression coverage are unchanged. Dev build job **114** published release **125** (`sha256 d6b3452bb043e480…`), synced to production release **49**, and rolling deploy job **22** completed **8/8** nodes on 0.4.3. Production health and fleet smoke passed; browse binary is synced at 0.4.3.
- **Status:** Resolved and instrumentation cleanup complete.
...
- **Problem:** Billing only counted "Protected Users" — licensed Member users reached via personal selection or Team/M365 Group membership. Guests, shared mailboxes, room/equipment mailboxes, and SharePoint site-only selections were **excluded**, letting MSPs back up recoverable data for those identities without a corresponding charge.
- **Design:** Approved spec [`Docs/specs/2026-07-21-protected-objects-billing-design.md`](specs/2026-07-21-protected-objects-billing-design.md) — bill **Protected Objects**: one Azure object ID, counted at most once per backup user, reached by personal selection **or** Team/Group membership **or** SharePoint site membership. Internal metric key `protected_users` and admin setting key `protected_user_price_cad` are **unchanged**; only customer/admin-facing labels changed to "Protected Objects" / "Protected Object price (CAD)".
- **Resolver (`ProtectedUserResolver.php`):** Removed exclusions in `isBillableMember` for guests, `#EXT#` UPNs, and `TYPE_MAILBOX` — all now bill like any other identity when reached by any billable path. Added `TYPE_SHAREPOINT_SITE` as a third membership source alongside Team/channel/M365 Group, with its own cache-first / on-demand-fetch / `member_resolution_pending` handling (`resolveSiteMembers()`), reusing the single `$protected[$azureId] = true` map as the sole dedupe structure (no double counting across personal + team + group + site).
- **Discovery (`DiscoveryService::listSiteMembers()`):** New method — paginates `GET /sites/{id}/permissions`, extracts user principals from `grantedToIdentitiesV2` / `grantedToIdentities` / `grantedToV2` / `grantedTo`, filters to billable Graph user members (devices/service principals skipped), writes an ops/debug snapshot to `discovery/site_members/{siteId}.json`. Uses the **existing `Sites.Read.All`** application permission — no new Graph permission needed (documented in `AZURE_SETUP.md`).
- **Inventory (`InventoryService::enrichSharePointSiteMembers()`):** New enrichment pass (mirrors the existing Team/Group member enrichment) that runs during inventory refresh, fetches/caches `meta.member_azure_ids` / `meta.member_count` / `meta.members_fetched_at` on each `sharepoint_site` resource, and merges in members from any Team/Group whose `meta.sharepoint_site_id` matches the site (so a Team-backed site resolves correctly even before its own permissions are fetched). Progress reporting reuses the existing `writeRefreshProgress` pattern.
- **Tests (`tests/ms365_protected_user_resolver_test.php`):** Rewrote prior exclusion assertions (guest/shared-mailbox-not-counted) to assert inclusion; added cases for SharePoint site members (including a guest site member), personally-selected guest, personally-selected room/equipment-style mailbox, and a cross-source dedupe case (same identity reached via personal selection + Team + Site → counted once). All green.
- **UI/copy (labels only; internal keys/JSON fields unchanged):** `e3backup_user_detail.tpl` (Usage & Billing drawer), `e3backup_pricing_panel.tpl`, `ms365_job_wizard.tpl` (billing dock + stale-inventory tooltip now mentions site membership), `ms365backup/assets/js/jobs.js` (admin Jobs column `Protected` → `Objects`), `Ms365BillingConfig::metricFriendlyName()`, `Ms365ProductBootstrap::METRICS` name + product description, `ms365backup.php` addon setting `protected_user_price_cad` FriendlyName/Description.
- **WHMCS config option migration:** `Ms365ProductBootstrap` now renames an existing `tblproductconfigoptions` row named "Protected Users" → "Protected Objects" in place (by `configid`) instead of creating a duplicate option, so already-bootstrapped products relabel cleanly (`options_renamed` in the bootstrap report).
- **Docs:** Updated `MS365_BILLING_AND_STORAGE_DESIGN.md` §2.1 (Protected Object definition, dedupe law, SharePoint no-longer-deferred) and §6/§6.1 (SharePoint site member resolution, `Sites.Read.All`, no new permission); `MS365_BILLING_GUIDE.md` (summary table, definition, who-counts table, dedup + worked examples, FAQ, glossary, changelog row); `AZURE_SETUP.md` (`Sites.Read.All` note covering site-permission metering, no additional permission required); this `PROGRESS.md` entry; legacy plan `/root/.cursor/plans/ms365_billing_design_cef1654c.plan.md` updated to Protected Objects terminology with SharePoint member metering marked in-scope (not deferred).
- **Status:** Code + tests implemented and uncommitted on branch `feature/ms365-protected-objects-billing` (not yet committed — awaiting explicit commit instruction). Docs in this session bring documentation in line with the implemented code.
- **Next steps:**
  1. Run an **inventory refresh** on real/test tenants so `meta.member_azure_ids` gets populated for existing SharePoint sites (cache is empty until refreshed; on-demand fetch covers the gap but a proactive refresh avoids `member_resolution_pending` on first view).
  2. **Watch the first `ms365_billing.php` meter run** after deploy — counts may jump for tenants with guests/shared mailboxes/SharePoint-only selections (no grandfathering, per design). Spot-check a few services' new `protected_users` qty against expectations before/after.
  3. Optional: run a **bootstrap pass** (`Ms365ProductBootstrap`) on any WHMCS install where the config option was already created, to pick up the in-place rename from "Protected Users" → "Protected Objects" (`options_renamed` in the report); verify no duplicate config option was created.
