...
- **Fix (worker 0.4.2):** Mail-only `paginateMailFolders` sets `SoftStopRepeatedNextLink` (separate from `DuplicatePageDetectOnly`) for repeated nextLink soft-stop (`PaginationOutcome.StoppedOnRepeatedNextLink`). SharePoint/calendar DetectOnly callers keep strict repeated-nextLink errors. Ladder retries at page sizes **100 → 50 → 25**: returns on natural completion; on wedge, logs warning and retries smaller; on final wedge, keeps unique folders returned and continues sync. Mail-folder monitor uses nil per-page logger; high-level `opts.Log` wedge warnings unchanged. Message-delta behavior unchanged. Session `6f5f7c` instrumentation preserved for post-deploy verification.
...
- **Status:** Production post-fix workload retry/E2E verification remains **pending operator reproduction**. Do not remove session `6f5f7c` instrumentation until the retry logs prove folder enumeration proceeds and the operator confirms.
