...
function jobsApp(opts) {
    ...
    return {
        ...
        async loadJobs() {
            ...
            try {
                ...
                if (this.scopeUserId != null) {
                    params.append('user_id', String(this.scopeUserId));
                } else {
        ...
        resolveBackupUserRouteId(job) {
            if (this.scopeUserId != null && String(this.scopeUserId) !== '') {
                return String(this.scopeUserId);
            }
            const routeId = job && job.backup_user_route_id ? String(job.backup_user_route_id) : '';
            return routeId;
        },
        goToRestores(job) {
            if (!job) return;
            const backupUserRouteId = this.resolveBackupUserRouteId(job);
            if (!backupUserRouteId) {
                e3backupNotify('error', 'This job is not linked to a backup user. Restore from the user detail page after assigning ownership.');
                return;
            }
            if (this.scopeUserId != null && String(this.scopeUserId) !== '') {
                window.dispatchEvent(new CustomEvent('eb-e3-user-restore-requested', {
                    detail: {
            ...
            url.searchParams.set('m', 'cloudstorage');
            url.searchParams.set('page', 'e3backup');
            url.searchParams.set('view', 'user_detail');
            url.searchParams.set('user_id', backupUserRouteId);
            if (job.job_id) {
                url.searchParams.set('restore_job_id', String(job.job_id));
            }
            url.hash = 'restore';
...
function localWizardSet(key, val) {
    ...
    if (key === 'engine') {
        ...
        const legacyEngineUi = ['ring-2', 'ring-[var(--eb-info-border)]', 'border-[var(--eb-info-border)]', 'bg-[var(--eb-info-bg)]'];
        buttons.forEach((btn) => {
            const e = btn.getAttribute('data-engine-btn');
            btn.classList.remove(...legacyEngineUi);
            if (e === val) {
...
function localWizardSubmit() {
    ...
    const backupUserField = document.querySelector('input[name="backup_user_public_id"]');
    if (backupUserField && backupUserField.value) {
        payload.backup_user_public_id = backupUserField.value;
    }
