{*
    Shared Enrollment Tokens panel.

    Extracted from the standalone Tokens page so it can be embedded both there
    and inside the Agents page (Enrollment Tokens tab). Requires the host page
    controller to supply: $isMspClient, $tenants, $token (CSRF).

    Renders its own Alpine component (tokensApp) and modals.
*}
