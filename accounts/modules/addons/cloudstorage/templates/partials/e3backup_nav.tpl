...
<div class="mb-6" x-data="{ downloadFlyoutOpen: false }">
    <nav class="inline-flex rounded-full bg-slate-900/80 p-1 text-xs font-medium text-slate-400" aria-label="e3 Cloud Backup Navigation">
        ...
        <a href="index.php?m=cloudstorage&page=e3backup&view=recovery_media"
           class="px-4 py-1.5 rounded-full transition {if $activeNav == 'recovery_media'}bg-slate-800 text-slate-50 shadow-sm{else}hover:text-slate-200{/if}">
            Media Builder
        </a>
