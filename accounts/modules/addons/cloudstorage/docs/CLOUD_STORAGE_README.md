...
- `bucketobjects.php` rows include both `name` (relative display label) and `key` (full S3 object key or folder prefix). UI actions that talk to S3 (download, delete, copy URL) must use `key`.
