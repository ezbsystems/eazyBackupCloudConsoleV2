...
## Local Agent Kopia Retention (repo-native queue + agent execution)

Local Agent Kopia retention uses a **repo-native** model—not the cloud cron/object-delete path. The cloud retention cron (`s3cloudbackup_retention.php`) and `CloudBackupController::applyRetentionPolicy()` are **Cloud Backup only**; Local Agent jobs are excluded.
...
See `KOPIA_RETENTION_ARCHITECTURE.md` for full details.
