...
if (is_null($product) || is_null($product->username)) {
	(new JsonResponse(['status' => 'fail', 'message' => 'Unable to remove lifecycle configuration. Please try again later.'], 200))->send();
	exit;
}
...
(new JsonResponse(['status' => 'success', 'message' => $res['message'] ?? 'Removed.']))->send();
exit;
