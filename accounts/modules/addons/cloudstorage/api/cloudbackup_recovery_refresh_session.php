...
function refreshSessionExpiry(object $tokenRow, string $runId, int $hours = 6, int $minRemainingMinutes = 30): string
{
    $currentExpiry = '';
...
$run = $hasRunIdPk
    ? Capsule::table('s3_cloudbackup_runs')->whereRaw('run_id = ' . UuidBinary::toDbExpr($runIdNorm))->first()
    : Capsule::table('s3_cloudbackup_runs')->where('run_uuid', $runIdNorm)->first();
if (!$run) {
    respond(['status' => 'fail', 'message' => 'Run not found'], 404);
}

$newExpiry = refreshSessionExpiry($tokenRow, $runIdNorm);
