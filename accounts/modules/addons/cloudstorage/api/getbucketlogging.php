...
$res = $service->getLoggingWithTempKey($bucketName, $owner, (string)$adminAccessKey, (string)$adminSecretKey);

if (($res['status'] ?? 'fail') !== 'success') {
    (new JsonResponse(['status' => 'fail', 'message' => $res['message'] ?? 'Unable to fetch logging status.'], 200))->send();
    exit;
}
