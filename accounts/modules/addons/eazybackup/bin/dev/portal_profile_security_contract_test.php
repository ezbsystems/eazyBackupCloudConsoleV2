...
$targets = [
    ...
    'profile update api file' => [
        ...
        'markers' => [
            ...
            'current profile lookup marker' => '->first([\'name\', \'email\']);',
            'no-op success marker' => 'if ((string) ($current->name ?? \'\') === $name && strtolower((string) ($current->email ?? \'\')) === $email) {',
            'duplicate pre-check marker' => "->whereRaw('LOWER(email) = ?', [\$email])",
            'duplicate error message marker' => "portal_json(['status' => 'fail', 'message' => 'Email already in use for this tenant'], 400);",
            'tenant user update marker' => "Capsule::table('s3_backup_tenant_users')",
            'duplicate exception catch marker' => 'catch (\Throwable $e) {',
