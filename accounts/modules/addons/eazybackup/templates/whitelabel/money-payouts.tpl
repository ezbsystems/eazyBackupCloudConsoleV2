...
{capture assign=ebPhContent}
          <div class="flex flex-col gap-4 border-b border-[var(--eb-border-subtle)] px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 class="eb-type-h2 tracking-tight text-[var(--eb-text-primary)]">Payouts</h1>
              <p class="eb-page-description mt-1">View and search Stripe Connect payouts.</p>
            </div>
            <div class="shrink-0">
              <button type="button" id="eb-refresh-payouts" class="eb-btn eb-btn-primary eb-btn-sm">Refresh last 30 days</button>
            </div>
          </div>
          <div class="p-6">
            <section class="eb-card-raised !p-0 overflow-hidden">
              ...
              <div class="px-6 py-6"
                ...
                <div class="eb-table-toolbar mb-4 flex flex-col gap-3 xl:flex-row xl:items-center">
                  ...
                  <input type="text" x-model="search" placeholder="Search payout ID, status, or currency" class="eb-toolbar-search w-full xl:w-80">
                </div>
                <div class="eb-table-shell">
                  <table class="eb-table min-w-full text-sm">
                    <thead>
                      <tr>
                        <th class="text-left"><button type="button" class="eb-table-sort-button" @click="setSort('payout')">Payout ID <span x-text="sortIndicator('payout')"></span></button></th>
                        ...
                        <th class="text-left"><button type="button" class="eb-table-sort-button" @click="setSort('arrival')">Arrival <span x-text="sortIndicator('arrival')"></span></button></th>
                      ...
                      {if $rows|@count > 0}
                        {foreach from=$rows item=row}
                          ...
                          <tr
                            ...
                            <td class="eb-table-mono">{$row.stripe_payout_id|default:'-'}</td>
                            <td>{$row.amount/100|string_format:'%.2f'}</td>
                            <td>{$row.currency|upper|default:'USD'}</td>
                            <td>
                              <span class="eb-badge eb-badge--dot {if $st=='paid'}eb-badge--success{elseif $st=='pending' || $st=='in_transit'}eb-badge--warning{elseif $st=='failed' || $st=='canceled'}eb-badge--danger{else}eb-badge--default{/if}">{$st}</span>
                            </td>
                            <td>{$row.arrival_date|date_format:'%Y-%m-%d'}</td>
                            <td>{$row.created|date_format:'%Y-%m-%d %H:%M'}</td>
                            <td>
                              {assign var=acct value=$msp.stripe_connect_id|default:''}
                              {if $acct}
                                <a href="https://dashboard.stripe.com/connect/accounts/{$acct}/payouts/{$row.stripe_payout_id}" target="_blank" rel="noopener" class="font-medium text-[var(--eb-primary)] hover:underline">Open</a>
                              {else}-{/if}
                            </td>
                          </tr>
                        {/foreach}
                        <tr x-ref="noResults" style="display: none;">
                          <td colspan="7" class="px-4 py-6 text-center text-sm text-[var(--eb-text-muted)]">No payouts matched the current filters.</td>
                        </tr>
                      {else}
                        <tr>
                          <td colspan="7" class="px-4 py-6 text-center text-sm text-[var(--eb-text-muted)]">No payouts found.</td>
                        </tr>
                      {/if}
                    </tbody>
                  </table>
