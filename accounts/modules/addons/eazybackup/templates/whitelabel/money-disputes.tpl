<div class="min-h-screen bg-slate-950 text-gray-100 overflow-x-hidden">
  <div class="container mx-auto max-w-full px-4 pb-8 pt-6">
    ...
    <div class="w-full max-w-full min-w-0 overflow-hidden rounded-3xl border border-slate-800/80 bg-slate-950/80 shadow-[0_18px_60px_rgba(0,0,0,0.6)] px-6 py-6">
      <div class="mb-6">
        <h2 class="text-2xl font-semibold text-white">Disputes</h2>
        <p class="text-xs text-slate-400 mt-1">View and search Stripe Connect disputes.</p>
      </div>
      <form method="get" action="{$modulelink}" class="mb-4">
        ...
        <div class="flex gap-2">
          <input type="text" name="q" value="{$q|escape}" placeholder="Search dispute id, status, currency" class="w-full max-w-md px-3 py-2.5 rounded-lg bg-slate-800 text-sm text-slate-100 placeholder:text-gray-400 outline-1 -outline-offset-1 outline-white/10 focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-sky-700 transition"/>
      ...
      <div class="mb-4">
        <button type="button" id="eb-refresh-disputes" class="inline-flex items-center px-4 py-2 rounded-lg text-sm font-semibold text-white bg-sky-600 hover:bg-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 focus:ring-offset-slate-900">Refresh last 30 days</button>
      </div>
      <div class="w-full max-w-full min-w-0 rounded-2xl border border-slate-800/80 bg-slate-900/70 p-5 shadow-lg overflow-hidden">
        <table class="min-w-full text-sm text-slate-300">
          <thead class="bg-slate-900/80 text-slate-300">
            <tr>
              <th class="px-4 py-3 text-left font-medium">Dispute ID</th>
              ...
              <th class="px-4 py-3 text-left font-medium">Evidence Due</th>
              <th class="px-4 py-3 text-left font-medium">Charge ID</th>
