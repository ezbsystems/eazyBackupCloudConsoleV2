...
>
    <div class="flex h-full flex-col">
        <div class="border-b border-[var(--eb-border-subtle)] p-4">
            {assign var=ebPhPendingCount value=$eb_ph_pending_signups_count|default:0}
            {assign var=ebPhBellVisible value=(!isset($eb_ph_show_signup_approvals) || $eb_ph_show_signup_approvals) && $ebPhPendingCount > 0}
            <div class="flex items-center gap-3" :class="sidebarCollapsed ? 'flex-col' : ''">
                <div class="flex min-w-0 items-center gap-3" :class="sidebarCollapsed && 'justify-center'">
                ...
                {if $ebPhBellVisible}
                    <a href="{$WEB_ROOT}/index.php?m=eazybackup&amp;a=ph-signup-approvals"
                       class="eb-ph-bell relative inline-flex items-center justify-center rounded-md p-1.5 text-[var(--eb-text-secondary)] hover:bg-[var(--eb-surface-hover)] hover:text-[var(--eb-text-primary)] transition-colors"
                       :class="sidebarCollapsed ? '' : 'ml-auto'"
                       title="{$ebPhPendingCount} signup{if $ebPhPendingCount != 1}s{/if} awaiting approval"
                       aria-label="{$ebPhPendingCount} signup{if $ebPhPendingCount != 1}s{/if} awaiting approval">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M14.857 17.082a23.848 23.848 0 0 0 5.454-1.31A8.967 8.967 0 0 1 18 9.75V9A6 6 0 0 0 6 9v.75a8.967 8.967 0 0 1-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 0 1-5.714 0m5.714 0a3 3 0 1 1-5.714 0" />
                        </svg>
                        <span class="absolute -top-1 -right-1 inline-flex min-w-[18px] items-center justify-center rounded-full bg-[var(--eb-status-warning)] px-1 text-[10px] font-semibold leading-[18px] text-white">
                            {if $ebPhPendingCount > 99}99+{else}{$ebPhPendingCount}{/if}
                        </span>
                    </a>
                {/if}
            </div>
        ...
        <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-3">
            ...
            {* Signup Approvals: surfaced via the count-badged bell in the sidebar
               header (see top of this partial), per-tenant button in the
               White-Label Tenants slide-over, and an MSP email on each new
               pending signup. The standalone sidebar link was removed because
               the queue is exception-driven and empty most of the time. *}
            ...
            <div x-data="{ catalogOpen: {if $catalogGroupActive}true{else}false{/if} }" class="space-y-0">
                ...
                <div x-show="catalogOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="transition-all duration-300" :class="sidebarCollapsed ? 'eb-sidebar-subnav--collapsed' : 'mt-2 space-y-1 ml-4 pl-4 border-l border-slate-700/50'">
                    <button type="button" @click="window.location.href='{$WEB_ROOT}/index.php?m=eazybackup&amp;a=ph-catalog-products'" class="eb-sidebar-sublink w-full min-w-0 cursor-pointer text-left {if $ebPhSidebarPage eq 'catalog-products'}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-2'" :title="sidebarCollapsed ? 'Products' : ''">
                        ...
                        <span x-show="!sidebarCollapsed" x-transition.opacity class="min-w-0">Products</span>
                    </button>
                    <button type="button" @click="window.location.href='{$WEB_ROOT}/index.php?m=eazybackup&amp;a=ph-catalog-plans'" class="eb-sidebar-sublink w-full min-w-0 cursor-pointer text-left {if $ebPhSidebarPage eq 'catalog-plans'}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-2'" :title="sidebarCollapsed ? 'Plans' : ''">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6A2.25 2.25 0 0 1 6 3.75h2.25A2.25 2.25 0 0 1 10.5 6v2.25a2.25 2.25 0 0 1-2.25 2.25H6a2.25 2.25 0 0 1-2.25-2.25V6ZM3.75 15.75A2.25 2.25 0 0 1 6 13.5h2.25a2.25 2.25 0 0 1 2.25 2.25V18a2.25 2.25 0 0 1-2.25 2.25H6A2.25 2.25 0 0 1 3.75 18v-2.25ZM13.5 6a2.25 2.25 0 0 1 2.25-2.25H18A2.25 2.25 0 0 1 20.25 6v2.25A2.25 2.25 0 0 1 18 10.5h-2.25a2.25 2.25 0 0 1-2.25-2.25V6Z" />
                        </svg>
                        <span x-show="!sidebarCollapsed" x-transition.opacity class="min-w-0">Plans</span>
                    </button>
                </div>
            </div>
            {/if}
            ...
            <div x-data="{ billingOpen: {if $billingGroupActive}true{else}false{/if} }" class="space-y-0">
                <button type="button" @click="billingOpen = !billingOpen" class="eb-sidebar-link w-full cursor-pointer text-left {if $billingGroupActive}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-4'" :title="sidebarCollapsed ? 'Billing' : ''">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                <div x-show="billingOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="transition-all duration-300" :class="sidebarCollapsed ? 'eb-sidebar-subnav--collapsed' : 'mt-2 space-y-1 ml-4 pl-4 border-l border-slate-700/50'">
                    <button type="button" @click="window.location.href='{$WEB_ROOT}/index.php?m=eazybackup&amp;a=ph-billing-subscriptions'" class="eb-sidebar-sublink w-full min-w-0 cursor-pointer text-left {if $ebPhSidebarPage eq 'billing-subscriptions'}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-2'" :title="sidebarCollapsed ? 'Subscriptions' : ''">
                        ...
                        <span x-show="!sidebarCollapsed" x-transition.opacity class="min-w-0">Subscriptions</span>
                    </button>
                    <button type="button" @click="window.location.href='{$WEB_ROOT}/index.php?m=eazybackup&amp;a=ph-billing-invoices'" class="eb-sidebar-sublink w-full min-w-0 cursor-pointer text-left {if $ebPhSidebarPage eq 'billing-invoices'}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-2'" :title="sidebarCollapsed ? 'Invoices' : ''">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                        </svg>
                        <span x-show="!sidebarCollapsed" x-transition.opacity class="min-w-0">Invoices</span>
                    </button>
                    <button type="button" @click="window.location.href='{$WEB_ROOT}/index.php?m=eazybackup&amp;a=ph-billing-payments'" class="eb-sidebar-sublink w-full min-w-0 cursor-pointer text-left {if $ebPhSidebarPage eq 'billing-payments'}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-2'" :title="sidebarCollapsed ? 'Payments' : ''">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z" />
                        </svg>
                        <span x-show="!sidebarCollapsed" x-transition.opacity class="min-w-0">Payments</span>
                    </button>
                </div>
            </div>
            {/if}
            ...
            <div x-data="{ moneyOpen: {if $moneyGroupActive}true{else}false{/if} }" class="space-y-0">
                <button type="button" @click="moneyOpen = !moneyOpen" class="eb-sidebar-link w-full cursor-pointer text-left {if $moneyGroupActive}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-4'" :title="sidebarCollapsed ? 'Money' : ''">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                <div x-show="moneyOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="transition-all duration-300" :class="sidebarCollapsed ? 'eb-sidebar-subnav--collapsed' : 'mt-2 space-y-1 ml-4 pl-4 border-l border-slate-700/50'">
                    <button type="button" @click="window.location.href='{$WEB_ROOT}/index.php?m=eazybackup&amp;a=ph-money-payouts'" class="eb-sidebar-sublink w-full min-w-0 cursor-pointer text-left {if $ebPhSidebarPage eq 'money-payouts'}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-2'" :title="sidebarCollapsed ? 'Payouts' : ''">
                        ...
                        <span x-show="!sidebarCollapsed" x-transition.opacity class="min-w-0">Payouts</span>
                    </button>
                    <button type="button" @click="window.location.href='{$WEB_ROOT}/index.php?m=eazybackup&amp;a=ph-money-disputes'" class="eb-sidebar-sublink w-full min-w-0 cursor-pointer text-left {if $ebPhSidebarPage eq 'money-disputes'}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-2'" :title="sidebarCollapsed ? 'Disputes' : ''">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z" />
                        </svg>
                        <span x-show="!sidebarCollapsed" x-transition.opacity class="min-w-0">Disputes</span>
                    </button>
                    <button type="button" @click="window.location.href='{$WEB_ROOT}/index.php?m=eazybackup&amp;a=ph-money-balance'" class="eb-sidebar-sublink w-full min-w-0 cursor-pointer text-left {if $ebPhSidebarPage eq 'money-balance'}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-2'" :title="sidebarCollapsed ? 'Balance & Reports' : ''">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18 9 11.25l4.306 4.306a11.95 11.95 0 0 1 5.814-5.518l2.74-1.22m0 0-5.94-2.281m5.94 2.28-2.28 5.941" />
                        </svg>
                        <span x-show="!sidebarCollapsed" x-transition.opacity class="min-w-0">Balance & Reports</span>
                    </button>
                </div>
            </div>
            {/if}
            ...
            <div x-data="{ stripeOpen: {if $stripeGroupActive}true{else}false{/if} }" class="space-y-0">
                <button type="button" @click="stripeOpen = !stripeOpen" class="eb-sidebar-link w-full cursor-pointer text-left {if $stripeGroupActive}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-4'" :title="sidebarCollapsed ? 'Stripe Account' : ''">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                <div x-show="stripeOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="transition-all duration-300" :class="sidebarCollapsed ? 'eb-sidebar-subnav--collapsed' : 'mt-2 space-y-1 ml-4 pl-4 border-l border-slate-700/50'">
                    <button type="button" @click="window.location.href='{$WEB_ROOT}/index.php?m=eazybackup&amp;a=ph-stripe-connect'" class="eb-sidebar-sublink w-full min-w-0 cursor-pointer text-left {if $ebPhSidebarPage eq 'stripe-connect'}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-2'" :title="sidebarCollapsed ? 'Connect & Status' : ''">
                        ...
                        <span x-show="!sidebarCollapsed" x-transition.opacity class="min-w-0">Connect & Status</span>
                    </button>
                    <button type="button" @click="window.location.href='{$WEB_ROOT}/index.php?m=eazybackup&amp;a=ph-stripe-manage'" class="eb-sidebar-sublink w-full min-w-0 cursor-pointer text-left {if $ebPhSidebarPage eq 'stripe-manage'}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-2'" :title="sidebarCollapsed ? 'Manage Account' : ''">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z" />
                        </svg>
                        <span x-show="!sidebarCollapsed" x-transition.opacity class="min-w-0">Manage Account</span>
                    </button>
                </div>
            </div>
            {/if}
            ...
            <div x-data="{ settingsOpen: {if $settingsGroupActive}true{else}false{/if} }" class="space-y-0">
                <button type="button" @click="settingsOpen = !settingsOpen" class="eb-sidebar-link w-full cursor-pointer text-left {if $settingsGroupActive}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-4'" :title="sidebarCollapsed ? 'Settings' : ''">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                <div x-show="settingsOpen" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="transition-all duration-300" :class="sidebarCollapsed ? 'eb-sidebar-subnav--collapsed' : 'mt-2 space-y-1 ml-4 pl-4 border-l border-slate-700/50'">
                    <button type="button" @click="window.location.href='{$WEB_ROOT}/index.php?m=eazybackup&amp;a=ph-settings-checkout'" class="eb-sidebar-sublink w-full min-w-0 cursor-pointer text-left {if $ebPhSidebarPage eq 'settings-checkout'}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-2'" :title="sidebarCollapsed ? 'Checkout & Dunning' : ''">
                        ...
                        <span x-show="!sidebarCollapsed" x-transition.opacity class="min-w-0">Checkout & Dunning</span>
                    </button>
                    <button type="button" @click="window.location.href='{$WEB_ROOT}/index.php?m=eazybackup&amp;a=ph-settings-tax'" class="eb-sidebar-sublink w-full min-w-0 cursor-pointer text-left {if $ebPhSidebarPage eq 'settings-tax'}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-2'" :title="sidebarCollapsed ? 'Tax & Invoicing' : ''">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        ...
                        <span x-show="!sidebarCollapsed" x-transition.opacity class="min-w-0">Tax & Invoicing</span>
                    </button>
                    <button type="button" @click="window.location.href='{$WEB_ROOT}/index.php?m=eazybackup&amp;a=ph-settings-email'" class="eb-sidebar-sublink w-full min-w-0 cursor-pointer text-left {if $ebPhSidebarPage eq 'settings-email'}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-2'" :title="sidebarCollapsed ? 'Email Templates' : ''">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 0 1-2.25 2.25h-15a2.25 2.25 0 0 1-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25m19.5 0v.243a2.25 2.25 0 0 1-1.07 1.916l-7.5 4.615a2.25 2.25 0 0 1-2.36 0L3.32 8.91a2.25 2.25 0 0 1-1.07-1.916V6.75" />
                        </svg>
                        <span x-show="!sidebarCollapsed" x-transition.opacity class="min-w-0">Email Templates</span>
                    </button>
                    <button type="button" @click="window.location.href='{$WEB_ROOT}/index.php?m=eazybackup&amp;a=ph-settings-portal-branding'" class="eb-sidebar-sublink w-full min-w-0 cursor-pointer text-left {if $ebPhSidebarPage eq 'settings-portal-branding'}is-active{/if}" :class="sidebarCollapsed && 'justify-center px-2'" :title="sidebarCollapsed ? 'Portal Branding' : ''">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M9.53 16.122a3 3 0 0 0-5.78 1.128 2.25 2.25 0 0 1-2.4 2.245 4.5 4.5 0 0 0 8.4-2.245c0-.399-.078-.78-.22-1.128Zm0 0a15.998 15.998 0 0 0 3.388-1.62m-5.043-.025a15.994 15.994 0 0 1 1.622-3.395m3.42 3.42a15.995 15.995 0 0 0 4.764-4.648l3.876-5.814a1.151 1.151 0 0 0-1.597-1.597L14.146 6.32a15.996 15.996 0 0 0-4.649 4.763m3.42 3.42a6.776 6.776 0 0 0-3.42-3.42" />
                        </svg>
                        <span x-show="!sidebarCollapsed" x-transition.opacity class="min-w-0">Portal Branding</span>
                    </button>
                </div>
            </div>
            {/if}
            ...
            <div x-data="{ portalOpen: false }" class="space-y-0">
                <button type="button" @click="portalOpen = !portalOpen" class="eb-sidebar-link w-full cursor-pointer text-left" :class="sidebarCollapsed && 'justify-center px-4'" :title="sidebarCollapsed ? 'Tenant Portal' : ''">
                    ...
                    <span x-show="!sidebarCollapsed" x-transition.opacity class="min-w-0 flex-1">Tenant Portal</span>
