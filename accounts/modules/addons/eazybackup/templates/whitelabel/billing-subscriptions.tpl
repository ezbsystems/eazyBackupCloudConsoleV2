        ...
        <main class="flex-1 min-w-0 overflow-x-auto">
          <div class="flex items-center justify-between border-b border-slate-800/60 px-6 py-4">
            <div>
              ...
              <p class="mt-1 text-sm text-slate-400">Search and view customer subscriptions linked to your Stripe Connect account.</p>
            </div>
          </div>
          <div class="p-6">
            <section class="rounded-2xl eb-bg-card ring-1 ring-white/10 p-4">
              <div class="px-6 py-5">
                <h2 class="text-lg font-medium">Customer Subscriptions</h2>
                <p class="mt-1 text-sm text-slate-400">Review billing relationships for customer tenants connected to your Stripe account.</p>
              </div>
                  ...
                  <div class="relative" @click.away="statusOpen = false">
                    ...
                    <div x-show="statusOpen"
                      ...
                      <template x-for="option in [
                        ...
                        <button type="button"
                                class="w-full px-4 py-2 text-left text-sm transition"
                                :class="statusFilter === option.value ? 'bg-slate-800/70 text-white' : 'text-slate-200 hover:bg-slate-800/60'"
                    ...
                    <thead class="bg-slate-900/80 text-slate-300">
                      <tr>
                        <th class="px-4 py-3 text-left font-medium">
                          <button type="button" class="inline-flex items-center gap-1 hover:text-white" @click="setSort('client')">Client <span x-text="sortIndicator('client')"></span></button>
                        </th>
                        <th class="px-4 py-3 text-left font-medium">
                          <button type="button" class="inline-flex items-center gap-1 hover:text-white" @click="setSort('plan')">Plan <span x-text="sortIndicator('plan')"></span></button>
                        </th>
                        <th class="px-4 py-3 text-left font-medium">
                          <button type="button" class="inline-flex items-center gap-1 hover:text-white" @click="setSort('price')">Price <span x-text="sortIndicator('price')"></span></button>
                        </th>
                        <th class="px-4 py-3 text-left font-medium">
                        ...
                        <th class="px-4 py-3 text-left font-medium">
                          <button type="button" class="inline-flex items-center gap-1 hover:text-white" @click="setSort('started')">Started <span x-text="sortIndicator('started')"></span></button>
                        </th>
                        <th class="px-4 py-3 text-left font-medium">
                          <button type="button" class="inline-flex items-center gap-1 hover:text-white" @click="setSort('stripe')">Stripe ID <span x-text="sortIndicator('stripe')"></span></button>
                        </th>
                        <th class="px-4 py-3 text-left font-medium">
                          <button type="button" class="inline-flex items-center gap-1 hover:text-white" @click="setSort('stripe_link')">Stripe <span x-text="sortIndicator('stripe_link')"></span></button>
                        </th>
                          ...
                          <tr class="hover:bg-slate-800/50"
                            ...
                            <td class="px-4 py-3 text-left">
                              {if $row.tenant_public_id|default:'' neq '' && $row.tenant_status|default:'' neq 'deleted'}
                                <a href="{$modulelink}&a=ph-tenant&id={$row.tenant_public_id|escape:'url'}" class="inline-block rounded-xl bg-slate-700 px-3 py-1.5 text-xs text-white hover:bg-slate-600">View Client</a>
                              {else}
                                <span class="inline-block rounded-xl border border-slate-700 px-3 py-1.5 text-xs text-slate-400">Deleted tenant</span>
                              {/if}
                            </td>
                          </tr>
                        {/foreach}
