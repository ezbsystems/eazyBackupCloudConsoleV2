...
$baseUrl = 'addonmodules.php?module=eazybackup&action=powerpanel&view=terms';
    ...
    if (!$tokenOk) {
        ...
        try {
            if ($postOp === 'create') {
                ...
                $vid = (int)Capsule::table('eb_tos_versions')->insertGetId([
                    'version' => $version,
                ...
                $existing = Capsule::table('eb_tos_versions')->where('id', $id)->first();
                if (!$existing) {
                ...
                if ((int)$existing->is_active === 0) {
                    ...
                    if ($version !== (string)$existing->version) {
                        $dup = Capsule::table('eb_tos_versions')
                ...
                Capsule::table('eb_tos_versions')->where('id', $id)->update($data);
                $vid = $id;
                $notice = '<div class="alert alert-success">Terms of Service version saved.</div>';
                $op = 'edit';
...
if ($op === 'edit' && $vid > 0) {
    $row = Capsule::table('eb_tos_versions')->where('id', $vid)->first();
    if (!$row) {
    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <strong>Edit Terms of Service — <?= $e((string)$row->version) ?></strong>
            ...
            <div class="panel-body">
                <form method="post" action="<?= $e($baseUrl) ?>">
                    ...
                    <div class="form-group">
                        ...
                        <p class="help-block">Full HTML will be shown on client “View TOS” links.</p>
                    </div>
