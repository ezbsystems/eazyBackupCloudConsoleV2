...
- **Daily cycle (`billing_cycle_days` = 1)** — Hyper-V, VMware, Proxmox, etc. Last billable day is the **UTC calendar date** of host `revoked_at` (converted from DB local time), or the last inventory day the booster was present **before quantity dropped to zero**. A still-positive latest inventory snapshot does not count as removal. Charges on that UTC revoke day remain billable; only dates after it → `overbilled_past_grace`.
...
- **Policy Status**: Accounts on configured CSW Policy IDs with last backup warning; cross-ref Active Services for billed warning/error accounts; Sections B/C reconcile AS device fees against Bill History (phantom vs verified)
